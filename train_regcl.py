...
def main(args, train_data, test_data, output_path, logging):
    ...
    grams_dict = {}
    if args.module == 'AugModule':
        filterd_layer_inputs = [f'sam.image_encoder.blocks.{i+args.begin}.attn.B' for i in range(5)] + \
                               [f'sam.image_encoder.blocks.{i+args.begin}.attn.C' for i in range(5)] + \
                               [f'A']
        ...
        filterd_layer_inputs = [f'sam.image_encoder.blocks.{i+args.begin}.attn.A' for i in range(5)] + \
                               [f'sam.image_encoder.blocks.{i+args.begin}.attn.B' for i in range(5)]
        filterd_layer_params = [
                                '.*\.attn\.A.*',
                                '.*\.attn\.B.*'
                                ]
    ...
    for id, key in enumerate(train_data):
        ...
        if os.path.exists(f'{output_path}/{key}.pth'):
            ...
            if args.meth == 'indi' or id == 0:
                pass  # Train the LoRA model directly on the current task
            elif args.meth == 'sequ':
    ...
    for id, key in enumerate(train_data):
        ...
        if args.rank==0:
            ...
            with torch.no_grad():
                ...
                x.load_lora_parameters(f'{output_path}/{key}.pth', args)
                logging.info(f'model {key} loaded on {next(x.parameters()).device}')
                param_lst.append(x)
                grams_lst.append(grams_dict[key])
        if id != 0:
            if args.cuda!=-1 or args.rank==0:
                net = copy.deepcopy(net_origin).to('cuda:0')
                logging.info(f"------Merging {params_name} together as merged_{key}-------")
...
if __name__ == '__main__':
    ...
    # output_path = f'log/Comparison/{args.module}/RegCL_{args.module}_{args.begin}_{args.end}_4_0005'
    # output_path = f'log/Comparison/{args.module}/{args.meth}_{args.begin}_{args.end}_4_0005'
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    # base_dir = f'log/Comparison/{args.module}/{args.meth}/{args.order}'
    base_dir = f'log/Comparison/{args.module}/regcl/{args.meth}/{args.order}'
    output_path = f'{base_dir}/{args.seed}_SEED_{current_time}'
